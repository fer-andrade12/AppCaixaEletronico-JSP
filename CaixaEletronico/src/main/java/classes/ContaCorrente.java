...
public class ContaCorrente {
	...
	public Integer AtualizarSaldo(Integer opcc) {
		try {
			...
			String Sql = "select saldo, nome from cliente where " + "agencia = " + agencia_cc + " and conta = "
					+ conta_cc;
			...
			if (rs.next()) // se agencia e conta existem
			{
				...
				if (tipo_cc == 1) {
					...
					Sql = "update cliente set saldo = " + "saldo + " + valor_cc + " where agencia = " + agencia_cc
							+ " and " + " conta = " + conta_cc;
	...
	public void RegistrarOcorrencia() {
		try {
			...
			/*
			 * // inclusão modo concatenação String Sql =
			 * "insert into contacorrente values(" + getNumero_doc_cc() + "," +
			 * getAgencia_cc() + "," + getConta_cc() + "," + getValor_cc() + "," +
			 * getTipo_cc() + ")";
			 */

			/*
			 * // inclusão modo parametrizado (@) String Sql = "insert into " +
			...
			String Sql = "insert into " + "contacorrente(numero_doc, agencia, conta, valor, tipo) "
					+ "values(?,?,?,?,?)";
			PreparedStatement comando = (PreparedStatement) conn.prepareStatement(Sql);
